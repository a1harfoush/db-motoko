...
const ChatPage = () => {
  ...
  const [messages, setMessages] = useState([{ id: 1, message: "Hello! I'm your AI fitness coach. I can analyze your InBody reports and provide personalized fitness advice. How can I assist you today?", isUser: false, timestamp: new Date() }]);
  ...
  const handleSendMessage = async (message) => {
    const messageId = Date.now() + Math.random(); // Ensure unique IDs
    const userMessage = { id: messageId, message, isUser: true, timestamp: new Date() };
    ...
    try {
      ...
      const aiMessageId = Date.now() + Math.random(); // Ensure unique IDs
      setMessages(prev => [...prev, { id: aiMessageId, message: aiText, isUser: false, timestamp: new Date() }]);
    } catch (error) {
      ...
      const errorMessageId = Date.now() + Math.random(); // Ensure unique IDs
      setMessages(prev => [...prev, { id: errorMessageId, message: errorMessage, isUser: false, timestamp: new Date(), isError: true }]);
    } finally {
      setIsTyping(false);
    }
  };
